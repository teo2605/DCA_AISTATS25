...
lambda_vec = [0, lambda_opt, -lambda_opt/2, -lambda_opt, lambda_opt/2, lambda_max];
...
for ll = 1 : length(lambda_vec)
    switch lambda_vec(ll)
        case lambda_opt
            leg_default{ll} = sprintf('$\\lambda^*=%.4f$',lambda_opt);
        case 0
            leg_default{ll} = sprintf('$\\lambda=0$');
        case lambda_max
            leg_default{ll} = sprintf('$\\lambda_{max}=%.4f$',lambda_max);
        otherwise
            leg_default{ll} = sprintf('$\\lambda=%.4f$',lambda_vec(ll));
    end    
end
leg_th = [sprintf('wc rate: $\\lambda=0$'), sprintf('wc rate: $\\lambda^*=%.4f$',lambda_opt), leg_default]; % including th. plots
...
ylim([1e-12 1])
xlim([0 1000])
% exportgraphics(ax, 'Sim_eta=0.5_kappa=0.02_several_splittings_avg_num_its_hypo.pdf');
%% Generate table of N_epsilon
fprintf('\t %d / %d successful runs \n\n', num_d_good, num_d);
T = array2table(N_eps);
row_names = cell(length(lambda_vec),1);
% Rows: {'lambda=0','lambda_*','-lambda_*','lambda_*/2','lambda_max=eta'}
for ll = 1 : length(lambda_vec);  row_names{ll} = sprintf('lambda = %.4f',lambda_vec(ll)); end
...
T.Properties.Description = 'Case 1';
disp(T)